...
description: 并行调度多个子代理、汇总产物、生成统一交付的通用流程。用于需要并行拆分任务、子代理编排或聚合输出的场景。
---

# Parallel Subagents 通用多实例流程
...
3. **调度脚本生成**
    - 创建一个调度脚本（如 `run_children.sh`）。脚本需：
      ...
      - 若子目标涉及大文件、长 diff 或大量回滚，请提前拆分为更小的子任务或缩窄 prompt 范围，避免单个进程跑满 10–15 分钟仍无产出。
      - 避免同一批子代理并行改动同一文件；如无法规避，明确拆分责任区间或改为串行执行，以防互相覆盖。
      - 在子进程启动前为其创建独立临时目录（如 `scratch/<id>/`），把路径通过环境变量或 prompt 明示给子代理，要求其将阶段性成果（提纲、引用索引、进度标记等）实时写入该目录；子代理必须把最终稿写入 `"$SCRATCH_DIR/deliverable.md"`，等待主控复制后再根据指示清理；若任务失败或被中断，则保留目录并写明失败原因。
      - 子进程结束后立即校验 `deliverable.md` 是否存在并非空；缺失时将 `last_message` 复制为占位，同时在日志中标注失败原因并考虑重派任务。
...
4. **子进程 Prompt 设计**
   - 动态生成 prompt 模板，包含：
     - 子目标的描述、输入数据和约束边界。
     - 提醒子代理在规划时限定搜索/抽取总轮数不超过 X 轮（根据任务复杂度决定，一般推荐10），必要信息齐全即可结束。
     - 指定结果需为自然语言 Markdown：包含任务结论、关键证据列表、引用链接，以及遇到错误时的 Markdown 段落说明与后续建议。
     - 明确写入规范：要求子代理把关键中间状态（阶段名称、引用列表、重要片段草稿等）同步记录到分配好的 `scratch/<id>/` 文件夹（可使用 `progress.json`、`notes.md` 等规范命名），最终成果必须写入 `"$SCRATCH_DIR/deliverable.md"`；主控复制完成前禁止删除该文件。若任务失败或被中断，则保留整个文件夹并写明失败原因。
...
5. **并行执行与监控**
   ...
   - 在每轮任务结束后检查 `scratch/` 下未清理的目录：若进程被标记为成功但目录仍存在，应立即查看其中文件并补做清理；若目录保留且任务失败，可利用残留进度文件辅助重试，并在最终报告中注明使用情况。
   - 复制产物后务必核对：确认 `child_outputs/<id>.md` 非空，且（如存在）与 `scratch/<id>/deliverable.md` 内容一致；若缺少 deliverable，应退回到 `last_message` 并在 `logs/<id>.log` 中写明回退原因。
